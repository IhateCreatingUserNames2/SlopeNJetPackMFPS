...
public class bl_FirstPersonController : bl_FirstPersonControllerBase
{
    ...
    private SloopMovement sloopMovement;
    private SloopJetpack sloopJetpack;
    ...
    protected override void Awake()
    {
        ...
        sloopMovement = GetComponent<SloopMovement>();
        sloopJetpack = GetComponent<SloopJetpack>();
    }
    ...
    public override void OnLateUpdate()
    {
        ...
        // A variável 'moveDirection' é usada para suavização (lerp).
        // Vamos aplicar o lerp aqui para suavizar o movimento final.
        moveDirection = Vector3.Lerp(moveDirection, targetDirection, acceleration * Time.deltaTime);

        // APLICA O MOVIMENTO: Multiplicamos o vetor de velocidade final por Time.deltaTime
    ...
    public void FixedUpdate()
    {
        ...
        GetInput(out float baseSpeed);
        speed = baseSpeed;

        if (sloopMovement != null)
        {
            // ===== CORREÇÃO: Passa AMBOS os inputs de pulo =====
            // jumpPressed = apertar uma vez (pulo)
            // bl_GameInput.Jump() = segurar (jetpack)
            bool isHoldingJump = bl_GameInput.Jump();
            targetDirection = sloopMovement.CalculateMovement(m_Input, speed, jumpPressed, isHoldingJump);
        }
        else
        {
            if (isClimbing && m_Ladder != null) { OnClimbing(); }
            else { Move(); }
        }

        // Reseta o input de 'toque' a cada frame.
        if (jumpPressed) { jumpPressed = false; }
        // m_Jump continua sendo resetado para a lógica de fallback.
        if (m_Jump) { m_Jump = false; }
    }
    #endregion
    ...
    void MovementInput()
    {
        // ===== CORREÇÃO: Detecta pulo para o SloopMovement =====
        if (bl_GameInput.Jump(GameInputType.Down))
        {
            jumpPressed = true;
            Debug.Log("MovementInput: jumpPressed = true"); // DEBUG
        }
        ...
        // Lógica de pulo antiga (fallback para quando SloopMovement não está ativo)
    ...
    {
        // ===== CORREÇÃO SKIING: Skiing é baseado em VELOCIDADE, não ângulo =====
        bool isCurrentlySkiing = (sloopMovement != null && sloopMovement.IsSkiing);

        // O CharacterController.isGrounded é a nossa fonte da verdade
        ...
        // Se o contador do coyote time ainda não zerou, fingimos que ainda estamos no chão
        bool isEffectivelyGrounded = m_CharacterController.isGrounded || coyoteTimeCounter > 0f;

        // A lógica de OnLand/OnFall agora considera o skiing
        if (!m_PreviouslyGrounded && isEffectivelyGrounded)
        {
            OnLand();
        }
        else if (m_PreviouslyGrounded && !isEffectivelyGrounded)
        {
            PostGroundVerticalPos = m_Transform.position.y;

            // SÓ marca como falling se NÃO está esquiando
            if (!isFalling && !isCurrentlySkiing)
            {
                isFalling = true;
        ...
        // Atualiza falling apenas se não está esquiando
        if (isFalling && !isCurrentlySkiing)
        {
            VerticalDirectionCheck();
        }
        else if (isCurrentlySkiing)
        {
            // Se está esquiando, reseta flags de queda
            isFalling = false;
            fallingTime = 0;
        }

        if (!isEffectivelyGrounded && !m_Jumping && m_PreviouslyGrounded)
        {
        ...
        m_PreviouslyGrounded = isEffectivelyGrounded;
    }
    ...
    void Move()
    {
        // ===== CORREÇÃO: Se SloopMovement está ativo, ele já controla tudo =====
        // Esta função só deve rodar se SloopMovement estiver null ou desativado
        if (sloopMovement != null)
        {
            // SloopMovement.CalculateMovement já é chamado no FixedUpdate
            // e o resultado é aplicado em targetDirection
            // Então aqui só precisamos aplicar a força de stick to ground
            if (m_CharacterController.isGrounded)
            {
                // Força mínima para manter no chão (sem afetar o skiing)
                if (targetDirection.y < -2f)
                {
                    targetDirection.y = -2f;
                }
            }
            return; // Sai - deixa o SloopMovement controlar
        }

        // ===== Lógica original para quando SloopMovement não está ativo =====
    ...
    void OnSurface()
    {
        // ===== CORREÇÃO: Se está usando SloopMovement, não aplica a física padrão =====
        // O SloopMovement já cuida de todo o movimento
        if (sloopMovement != null)
        {
            return; // Deixa o SloopMovement controlar completamente
        }

        // ===== Lógica original apenas para quando SloopMovement está desativado =====
    ...
    void OnLand()
    {
        ...
        // ===== CORREÇÃO SKIING ESTILO TRIBES =====
        // Se o jogador está esquiando, NÃO aciona animação de queda
        bool wasSkiing = (sloopMovement != null && sloopMovement.IsSkiing);
        bool isCurrentlySkiing = (sloopMovement != null && sloopMovement.IsSkiing);

        // Se estava esquiando OU está esquiando OU tem muito momentum horizontal
        if (lastState == PlayerState.Skiing ||
            lastState == PlayerState.Sliding ||
            wasSkiing ||
            isCurrentlySkiing ||
            VelocityMagnitude > runSpeed)
        {
            // Reseta flags sem acionar animação de queda
            isFalling = false;
            m_Jumping = false;
            fallingTime = 0;
            ...
            // Som baixo apenas para feedback de contato
            PlayLandingSound(0.1f);

            // Mantém o estado de skiing se ainda estiver esquiando
            if (isCurrentlySkiing)
            {
            ...
            return; // Sai SEM acionar a animação de queda
        }
        // ===== FIM DA CORREÇÃO SKIING =====

        // Lógica normal de aterrissagem para quedas reais
    ...
    void SlopeControl()
    {
        // ===== CORREÇÃO: NÃO aplicar força de deslizamento se está usando o sistema de skiing =====
        // O SloopMovement já cuida da física de rampas
        if (sloopMovement != null)
        {
            return; // Sai da função - deixa o SloopMovement controlar
        }

        float angle = Vector3.Angle(Vector3.up, surfaceNormal);

        // Só aplica a física de deslizamento em rampas MUITO íngremes (acima do slopeLimit)
    ...
    private void GetInput(out float outputSpeed)
    {
        ...
        if (State != PlayerState.Climbing && State != PlayerState.Sliding && !isCurrentlySkiing)
        {
        ...
        if (isCurrentlySkiing)
        {
            // Aumenta o stepOffset para passar por cima de pequenos obstáculos
            m_CharacterController.stepOffset = 0.8f;
        }
        else if (State != PlayerState.Jumping)
        {
            // Restaura o stepOffset padrão quando não está esquiando
            m_CharacterController.stepOffset = defaultStepOffset;
        }
    ...
    public override bool isGrounded
    {
        get
        {
            // Se está esquiando, usa apenas CharacterController.isGrounded (sem coyote time)
            bool isCurrentlySkiing = (sloopMovement != null && sloopMovement.IsSkiing);
            if (isCurrentlySkiing)
            {
            ...
            // Caso contrário, usa o coyote time normalmente
            return coyoteTimeCounter > 0f || m_CharacterController.isGrounded;
        }
    }
